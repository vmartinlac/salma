...
normalize = True
if normalize:
    a0n = a0tmp / (a1tmp**2 + a2tmp**2 + a3tmp**2 + a0tmp**2)**0.5
    a1n = a1tmp / (a1tmp**2 + a2tmp**2 + a3tmp**2 + a0tmp**2)**0.5
    a2n = a2tmp / (a1tmp**2 + a2tmp**2 + a3tmp**2 + a0tmp**2)**0.5
    a3n = a3tmp / (a1tmp**2 + a2tmp**2 + a3tmp**2 + a0tmp**2)**0.5
else:
    a0n = a0tmp
    a1n = a1tmp
    a2n = a2tmp
...
for i in range(len(Xn)):
    pre = X[i]
    post = Xn[i]
    print("grad " + str(pre) + "' is:")
    print(sympy.derive_by_array(post, X))
